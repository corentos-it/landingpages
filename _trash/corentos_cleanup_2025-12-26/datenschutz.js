...
function setMeta(content, lang) {
  document.title = content.legal.privacy.metaTitle[lang];

  const metaDescription = document.querySelector('meta[name="description"]');
  if (metaDescription) {
    metaDescription.setAttribute('content', content.legal.privacy.metaDescription[lang]);
  }

  document.documentElement.setAttribute('lang', lang);
}

function buildPrivacy(content, lang) {
  const itemsHtml = content.legal.privacy.sections
    .map((section) => {
  ...
  return `
    <section class="pt-24 pb-20">
      <div class="max-w-6xl mx-auto px-4">
        <h1 class="text-3xl sm:text-4xl font-bold text-center mb-4"><span class="text-accent-400">${content.legal.privacy.title[lang]}</span></h1>
        <div class="w-20 h-1 bg-accent-500 mx-auto mb-12"></div>
        <div class="text-gray-500 text-sm mb-8">${content.legal.privacy.updatedLabel[lang]}: ${content.legal.privacy.updatedValue}</div>
        <div class="grid gap-8">${itemsHtml}</div>
        <div class="text-gray-500 text-sm mt-8">${content.legal.privacy.disclaimer[lang]}</div>
      </div>
    </section>
  `;
}
...
async function init() {
  ...
  function render(lang) {
    ...
    contentSlot.innerHTML = buildPrivacy(content, lang);
